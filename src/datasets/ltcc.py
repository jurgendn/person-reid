...
class LTCC(object):
    ...
    dataset_dir = ""
    def __init__(self, root: str = "data", **kwargs):
        ...
        logger.info(
            "  query    | {:5d} | {:8d} |".format(num_test_pids, num_query_imgs)
        )
        logger.info(
    ...
    def _process_dir_test(self, query_path, gallery_path):
        query_img_paths = glob.glob(os.path.join(query_path, "*.png"))
        gallery_img_paths = glob.glob(os.path.join(gallery_path, "*.png"))
