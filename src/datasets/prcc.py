...
class PRCC(object):
    ...
    def __init__(self, root="data", **kwargs):
        ...
        val, num_val_pids, num_val_imgs, num_val_clothes, _ = self._process_dir_train(
            self.val_dir
        )
        ...
        logger.info(
            "  val         | {:5d} | {:8d} | {:9d}".format(
                num_val_pids, num_val_imgs, num_val_clothes
            )
        )
        logger.info(
            "  test        | {:5d} | {:8d} | {:9d}".format(
                num_test_pids, num_test_imgs, num_test_clothes
            )
        )
        logger.info(
            "  query(same) | {:5d} | {:8d} |".format(num_test_pids, num_query_imgs_same)
        )
        logger.info(
            "  query(diff) | {:5d} | {:8d} |".format(num_test_pids, num_query_imgs_diff)
        )
        logger.info(
            "  gallery     | {:5d} | {:8d} |".format(num_test_pids, num_gallery_imgs)
        )
        logger.info("  --------------------------------------------")
        logger.info(
            "  total       | {:5d} | {:8d} | {:9d}".format(
    ...
    def _process_dir_train(self, dir_path):
        ...
        clothes2label = {
            clothes_id: label for label, clothes_id in enumerate(clothes_container)
        }
        ...
        for pdir in pdirs:
            ...
            for img_dir in img_dirs:
                ...
                if cam in ["A", "B"]:
                    ...
                    clothes_id = clothes2label[
                        osp.basename(pdir) + osp.basename(img_dir)[0]
                    ]
            ...
            for pdir in pdirs:
                ...
                for img_dir in img_dirs:
                    ...
                    if cam == "A":
                        ...
                        clothes_id = pid2label[pid] * 2
                        query_dataset_same_clothes.append(
                            (img_dir, pid, camid, clothes_id)
                        )
                    else:
                        clothes_id = pid2label[pid] * 2 + 1
                        query_dataset_diff_clothes.append(
                            (img_dir, pid, camid, clothes_id)
                        )
