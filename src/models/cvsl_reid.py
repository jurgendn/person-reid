...
from config_refactored import LossConfig, ModelConfig
from src.losses import build_losses
from src.models.modules.classifier import NormalizedClassifier
...
class CVSLReID(nn.Module):
    def __init__(self, model_config: ModelConfig, loss_config: LossConfig):
        super(CVSLReID, self).__init__()
        self.config = model_config
        self.ft_net = FTNet(
            target_layer=model_config.ft_net_config.target_layer,
            output_layer_name=model_config.ft_net_config.output_layer_name,
        )
        self.fusion = FusionNet(
            fusion_net_apprearance_dim=model_config.fusion_config.fusion_net_apprearance_dim,
            fusion_net_output_dim=model_config.fusion_config.fusion_net_output_dim,
            fusion_net_shape_dim=model_config.fusion_config.fusion_net_shape_dim,
        )
        self.id_classifier = NormalizedClassifier(
            feature_dim=model_config.fusion_config.fusion_net_output_dim,
            num_classes=model_config.classifier_num_classes,
        )
        if model_config.train_shape is True:
            self.shape_embedding = ShapeEmbedding(
                pose_n_features=model_config.shape_encoder_config.pose_n_features,
                n_hidden=model_config.shape_encoder_config.n_hidden,
                out_features=model_config.shape_encoder_config.out_features,
                relation_layers=model_config.shape_encoder_config.relation_layers,
                aggregation=model_config.shape_encoder_config.aggregation,
            )
        losses = build_losses(config=loss_config)
        if loss_config.use_clothes_loss is True:
            self.clothes_loss = losses["clothes"]
            self.cal = losses["cal"]
            self.clothes_classifier = NormalizedClassifier(
                feature_dim=model_config.fusion_config.fusion_net_output_dim,
                num_classes=model_config.classifier_num_classes,
            )
    ...
    def forward(
        ...
        x_pose_features = x_pose_features.to(self.dummy_param.device)
        edge_index = edge_index.to(self.dummy_param.device)

        appearance_feature = self.ft_net(x=x_image)

        if self.config.train_shape:
...
#         self.losses = build_losses(BASIC_CONFIG)
...
if __name__ == "__main__":
    from global_var import loss_config, model_config

    reid_model = CVSLReID(model_config=model_config, loss_config=loss_config)
    print(reid_model)
